...
// 折线图
const lineChartAndPie = computed(() => {
  let lineChart = {
    previousYear: [],
    thisYear: [],
    pieChart: [],
  };
  if (inLineChartAndPieChart.value) {
    lineChart.thisYear = inLineChartAndPieChart.value.thisYear;
    lineChart.previousYear = inLineChartAndPieChart.value.previewYear;
    lineChart.pieChart = inLineChartAndPieChart.value.thisYear.map((value, index, array) => {
...
const initPieChart = () => {
  ...
  const getvalue = lineChartAndPie.value.pieChart;
  const data = [];
...
const inLineChartAndPieChart = ref({
  previewYear: [],
  thisYear: [],
  pieChart: [],
})
const getHomeIpStatistics = async () => {
  ...
  if (response.code !== 0) {
    ...
    inLineChartAndPieChart.value.previewYear = response.data.groupPreviousYear.map((obj) => Number(obj.ip_access));
    inLineChartAndPieChart.value.thisYear = response.data.groupThisYear.map((obj) => Number(obj.ip_access));
