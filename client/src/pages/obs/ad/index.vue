...
<script setup lang="ts" name="/ad">
	...
	import { getAdList,deleteAd,changeAdStatus } from '/@/api/api';
	...
	const changeStatusFun = async (row : Object,status:number) => {
		let res = await changeAdStatus({id:row.id,status:status});
	...
	const state = reactive({
		tableData: {
			...
			// 表头内容（必传，注意格式）
			header: [
				{ key: 'img', colWidth: '', title: '广告图',width:150,height:100, type: 'image', isCheck: true },
				{ key: 'sort', colWidth: '', title: '占用的位置', type: 'text', isCheck: true },
				{ key: 'link', colWidth: '', title: '链接', type: 'text', isCheck: true },
				{ key: 'mark', colWidth: '', title: '备注', type: 'tag', isCheck: true },
				{ key: 'status', colWidth: '', title: '状态', fun:changeStatusFun, type: 'switch', isCheck: true },
	...
	const getTableData = async (e = {}) => {
		...
		const data = [];
		let row = await getAdList({...e,...state.tableData.page});
	...
	const deleteRow = (row:object) => {
		ElMessageBox.confirm(`此操作将永久删除广告图，是否继续?`, '提示', {
